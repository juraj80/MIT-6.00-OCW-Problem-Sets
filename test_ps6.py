...
def test_pick_best_word():
    ...
    # test 1
    hand = {'q':1, 's':1, 'u':1, 'w':1, 'x':1, 'j':1, 'o':1}
    failure = False
    word = pick_best_word(hand,POINTS_DICT)
    bestWord = 'jows'
    if POINTS_DICT[word] != POINTS_DICT[bestWord]:
        print "FAILURE: test_pick_best_word"
        print "\tExpected ", bestWord, " score :", POINTS_DICT[bestWord],"instead got ",word," score :", POINTS_DICT[word]  
        failure = True

    # test 2
    hand = {'i':1,'a':1,'k':2,'q':1,'l':2}
    word = pick_best_word(hand, POINTS_DICT)
    bestWord = 'klik'
    if POINTS_DICT[word] != POINTS_DICT[bestWord]:
        print "FAILURE: test_pick_best_word"
    ...
    # test 3
    hand = {'y':1,'e':1,'d':1,'f':1,'i':1,'h':1,'n':1}
    word = pick_best_word(hand, POINTS_DICT)
    bestWord = 'edify'
    if POINTS_DICT[word] != POINTS_DICT[bestWord]:
        print "FAILURE: test_pick_best_word"
...
def test_pick_best_word_runtime():
    start_time = time.time()
    hand = {'a':1,'e':1,'r':1,'t':2,'x':1,'k':1}
    word = pick_best_word(hand,POINTS_DICT)
    end_time = time.time()
    total_time = end_time - start_time
    print "Total time pick_best_word ",total_time

def test_pick_best_word_faster_runtime():
    start_time = time.time()
    hand = {'a':1,'e':1,'r':1,'t':2,'x':1,'k':1}
    word = pick_best_word_faster(hand,REARRANGE_DICT)
    end_time = time.time()
    total_time = end_time - start_time
    print "Total time pick_best_word_faster ",total_time
...
test_is_valid_word()
print "----------------------------------------------------------------------"
print "Testing pick_best_word..."
test_pick_best_word()
print "----------------------------------------------------------------------"
print "Comparing pick_best_word_runtime..."
test_pick_best_word_runtime()
test_pick_best_word_faster_runtime()
